...
Отказоустойчивость: если master упал — можно (после процедуры failover) поднять slave как новый master и быстро восстановить работу.

Резерв для аварий: данные почти полностью продублированы (зависит от задержки репликации).

Безопасное обслуживание master: часть задач можно делать на slave (бэкапы, проверки, отчёты), не трогая боевой master.
...
Важно понимать: чтение с slave возможно, но обычно в такой схеме slave держат “как запаску”, чтобы не усложнять консистентность.
...
Ниже — понятный план для новичка, где используются оба подхода:

0) Коротко: что такое вертикальный и горизонтальный шардинг
Вертикальный шардинг = “разносим разные таблицы (или разные части данных) по разным базам/серверам”.
...
Важно: при шардинге JOIN между users и books внутри одной SQL-команды обычно не делают, потому что таблицы в разных БД/шардах. Делают “склейку” на уровне приложения.

4) Блок-схема (где что расположено)
Можно вставить как “блоксхему” (Mermaid). Если ваш портал Mermaid не поддерживает — ниже дам ещё ASCII.
```
[Client] -> [API]
              |-> [Users DB]  (users)
              |-> [Stores DB] (stores)
...
```
Как в приложении выбирать shard (логика маршрутизации)
(просто принцип, на словах)
